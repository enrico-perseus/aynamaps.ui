...
@Component({
  selector: 'app-top-segments',
  templateUrl: './top-segments.component.html',
  styleUrls: ['./top-segments.component.css']
})
export class TopSegmentsComponent implements OnInit {
  ...
  getPlayers(){
    let self = this;
    this.locationService.getPlayer().toPromise()
    .then(
      res => {                
        self.players = res;
        self.viewPlayers = [];
        for (let i = 0; i < self.players.length; i++){
           self.viewPlayers.push({id: self.players[i].player_code, name: self.players[i].player_number}); 
        }
        console.log(self.viewPlayers);
      },
      err => {        
