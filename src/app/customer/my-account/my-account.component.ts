...
export class MyAccountComponent implements OnInit {
  ...
  reset() {
    ...
    this.authService.getCurrentUser().then(user => {      
      ...
      self.uid = self.currentUser['_id'];
      self.email = self.currentUser['email'];
      self.role = self.currentUser['role'];
      if (self.currentUser['avatar'] == '' || self.currentUser['avatar'] == undefined) {
        self.currentUser['avatar'] = 'assets/uploads/default_profile.png';
      }
  ...
  saveProfileInfo(avatarUrl) {
    ...
    this.authService.changeCustomerPassword(this.name, this.OldPassword, this.NewPassword, avatarUrl, function(err) {
      if (err == null) {
        self.authService.getCurrentUser().then(user => {
          self.currentUser = user;
          self.router.navigate(['/customer/home', { outlets: { popup: ['users'] } }]);
          //self.router.navigate(['home/users']);
        });
      } else {
    ...
    /* this.customerService.changePassword(this.currentUser, this.name, this.OldPassword, this.NewPassword, avatarUrl).then(
